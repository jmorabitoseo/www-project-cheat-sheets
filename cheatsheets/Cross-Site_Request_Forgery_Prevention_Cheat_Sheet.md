...
layout: col-document
title: Cross-Site Request Forgery (CSRF Attack) Prevention Cheat Sheet
...
Any state changing operation requires a secure random token (e.g., CSRF token) to prevent CSRF attacks. A CSRF token should be:

- unique per user session;
- large random value;
- generated by a Cryptographically Secure Pseudo-Random Number Generator (CSPRNG).

The CSRF token can be added through hidden fields, headers, and can be used with forms, and AJAX calls. Make sure that the token is not leaked in the server logs, or [in the URL](#disclosure-of-token-in-url). The server must reject the requested action if the CSRF token fails validation.
